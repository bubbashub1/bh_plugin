...
(function(){'use strict';function shareUrl(raw,title,button){var u;try{u=new URL(raw||window.location.href,window.location.origin)}catch(e){u=new URL(window.location.href)}var a=['bh_search','bh_category','bh_age_range','bh_region','bh_town','bh_saved_location','bh_day','bh_price','bh_free_activity','bh_view','bh_date'],c=new URL(u.origin+u.pathname);a.forEach(function(k){var v=u.searchParams.get(k);if(v)c.searchParams.set(k,v)});var opts={title:title||'Bubba Hub family activities',text:'Have a look at these family activities on Bubba Hub',url:c.toString()};if(button&&button.getAttribute('data-bh-admin-share')==='1'){var choice=window.prompt('Share to: Facebook, WhatsApp, X, Email or Copy Link');var v=(choice||'').toLowerCase().trim();var links={facebook:'https://www.facebook.com/sharer/sharer.php?u='+encodeURIComponent(c.toString()),whatsapp:'https://wa.me/?text='+encodeURIComponent(opts.text+' '+c.toString()),x:'https://twitter.com/intent/tweet?text='+encodeURIComponent(opts.text)+'&url='+encodeURIComponent(c.toString()),email:'mailto:?subject='+encodeURIComponent(opts.title)+'&body='+encodeURIComponent(opts.text+'\n\n'+c.toString())};if(v==='copy link'){if(navigator.clipboard)navigator.clipboard.writeText(c.toString()).then(function(){alert('Share link copied to your clipboard.')});return}if(links[v]){window.open(links[v],'bhshare','width=700,height=600,noopener,noreferrer')}return}if(navigator.share){button.classList.add('is-sharing');navigator.share(opts).catch(function(){}).finally(function(){button.classList.remove('is-sharing')});return}if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(c.toString()).then(function(){alert('Share link copied to your clipboard.')});return}window.prompt('Copy this share link:',c.toString())}document.addEventListener('click',function(e){var b=e.target.closest('[data-bh-share-search]');if(b){e.preventDefault();shareUrl(window.location.href,'Bubba Hub family activities',b)}});document.addEventListener('submit',function(e){var f=e.target.closest('.bh-directory-search__save-form');if(!f)return;var b=f.querySelector('button[type="submit"]');if(!b||b.disabled)return;b.disabled=true;b.classList.add('is-saving');b.innerHTML='<span class="bh-directory-search__spinner" aria-hidden="true"></span>Saving…'})}());