...
final class Schedule {
    ...
    /**
     * Day is intentionally sourced only from the ACF weekly schedule.
     */
    public static function matches_day(int $post_id, string $day = ''): bool {
        if ($day === '') {
            return true;
        }
        ...
        foreach (Listing::get_schedule($post_id) as $row) {
            ...
            if (sanitize_title($row_day) === sanitize_title($day)) {
                return true;
            }
        }
    ...
    /**
     * Kept for compatibility with existing callers. Term Time is now
     * filtered from Directorist metadata, not from the ACF schedule.
     */
    public static function matches(int $post_id, string $day = '', string $term_time = ''): bool {
        return self::matches_day($post_id, $day);
    }
